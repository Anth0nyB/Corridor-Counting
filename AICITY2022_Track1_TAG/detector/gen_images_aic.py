...
def preprocess(src_root, dst_root):
    ...
    src_list = ['validation/S05']
    dst_dir_list = [ dst_root + i for i in src_list]
    ...
    for i,x in enumerate(src_list):
        ...
        for y in os.listdir(x_path):
            ...
            video_path = os.path.join(y_path,'vdo.avi')
            roi_path = os.path.join(y_path, 'roi.jpg')
            ignor_region = cv2.imread(roi_path)

            dst_img1_dir = os.path.join(dst_dir_list[i],y)
            ...
            while frame_current<frame_count-1:
                ...
                if not os.path.isfile(dst_f_path):
                    ...
                    print('{}:{} generated to {}'.format(y, dst_f, dst_img1_dir))
                else:
                    print('{}:{} already exists.'.format(y, dst_f))
